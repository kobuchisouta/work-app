...
type WatchedDoc = {
    ...
    progress?: number; // 0〜1
    lastWatchedAt?: any; // ✅ Firestore Timestamp を想定（toDate()がある）
};
...
export default function ProfilePage() {
    ...
    const { skillPie, progressPie, weeklySeries } = useMemo(() => {
        // ✅ まずは「絶対にグラフが動く」ため、HTML/JS/CSSも含める
        ...
        for (const d of watched) {
            ...
            // ✅ Timestamp を正しく Date に変換
            const dt: Date | null = d.lastWatchedAt?.toDate?.() ?? null;
            if (dt) {
                const wk = weekKey(dt);
                    ...
                    <>
                        ...
                        <div style={{ marginTop: 16 }}>
                            ...
                            {weeklySeries.length === 0 ? (
                                <p>まだ学習履歴がありません（動画を見て閉じると自動保存されます）</p>
                            ) : (
                        ...
                        <div style={{ marginTop: 8, fontSize: 12, opacity: 0.7 }}>
                            watchedVideos 件数: {watched.length}
                        </div>
                    </>
                )}
            </section>
