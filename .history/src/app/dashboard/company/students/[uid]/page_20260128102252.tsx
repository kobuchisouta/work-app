...
export default function CompanyStudentPortfolioPage({
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                // doc.id を videoId として持たせる（存在しない場合に備えて）
    ...
    const {
        ...
        const completedList: WatchedDoc[] = [];
        const inProgressList: WatchedDoc[] = [];
        ...
        // 並び替え：最近見た順
        const sortByLastWatchedDesc = (a: WatchedDoc, b: WatchedDoc) => {
            const as = a.lastWatchedAt?.seconds ?? 0;
            const bs = b.lastWatchedAt?.seconds ?? 0;
            return bs - as;
        };

        completedList.sort(sortByLastWatchedDesc);
        inProgressList.sort(sortByLastWatchedDesc);
        ...
        return {
            ...
            completedVideos: completedList,
            inProgressVideos: inProgressList,
        };
    }, [watched]);
                            ...
                            <div className={styles.grid2} style={{ marginTop: 14 }}>
                                ...
                                <div className={styles.listCard}>
                                    <div className={styles.listHead}>
                                        <h3 className={styles.listTitle}>✅ 視聴完了</h3>
                                        <span className={styles.listCount}>{completedVideos.length} 本</span>
                                    </div>

                                    {completedVideos.length === 0 ? (
                                        <p className={styles.muted}>完了した動画はまだありません。</p>
                                    ) : (
                                        <ul className={styles.videoList}>
                                            {completedVideos.map((v) => (
                                                <li key={v.videoId ?? v.title ?? Math.random()} className={styles.videoItem}>
                                                    <div className={styles.videoTop}>
                                                        <p className={styles.videoName}>{v.title || "（タイトルなし）"}</p>
                                                        <span className={styles.videoPct}>{Math.round((v.progress ?? 0) * 100)}%</span>
                                                    </div>

                                                    <p className={styles.videoMeta}>
                                                        {v.skillTags?.length ? v.skillTags.join(" / ") : "タグなし"}
                                ...
                                <div className={styles.listCard}>
                                    <div className={styles.listHead}>
                                        <h3 className={styles.listTitle}>⏳ 視聴途中</h3>
                                        <span className={styles.listCount}>{inProgressVideos.length} 本</span>
                                    </div>

                                    {inProgressVideos.length === 0 ? (
                                        <p className={styles.muted}>途中の動画はありません。</p>
                                    ) : (
                                        <ul className={styles.videoList}>
                                            {inProgressVideos.map((v) => {
                                                ...
                                                return (
                                                    <li key={v.videoId ?? v.title ?? Math.random()} className={styles.videoItem}>
                                                        <div className={styles.videoTop}>
                                                            <p className={styles.videoName}>{v.title || "（タイトルなし）"}</p>
                                                            <span className={styles.videoPct}>{pct}%</span>
                                                        </div>

                                                        <div className={styles.progressBar}>
                                                            <div className={styles.progressFill} style={{ width: `${Math.max(1, pct)}%` }} />
                                                        </div>
